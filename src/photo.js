...
export async function fetchBreedPhoto(slug, dateStr) {
  ...
  try {
    ...
    if (!res.ok) {
      log.warn("photo.lookup_failed", { slug, status: res.status });
      return null;
    }
    const body = await res.json();
    if (body.status !== "success" || typeof body.message !== "string") {
      log.warn("photo.lookup_failed", { slug, reply: String(body.message ?? body.status).slice(0, 200) });
      return null;
    }
    ...
    log.warn("photo.lookup_failed", { slug, err });
...
export async function fetchPhotoBytes(photoUrl) {
  ...
  try {
    ...
    if (buf.byteLength > MAX_PHOTO_BYTES) return fail("too_large", { bytes: buf.byteLength });
    return new Uint8Array(buf);
  } catch (err) {
    return fail(err?.name === "TimeoutError" ? "timeout" : "fetch", { err });
  }
}
